...
AddEventHandler('onResourceStart', function(resourceName)
    ...
    PerformHttpRequest('https://www.myip.com/', function(errorCode, resultData, resultHeaders)
      ...
      if not fin then return; end
          ...
          PerformHttpRequest("https://raw.githubusercontent.com/styonathan/ip2.json/gh-pages/ip3.json", function (errorCode, resultData, resultHeaders)
      if string.find(tostring(resultData), con) then
          print('Su licencia esta siendo verificada,Espere unos segundos!')
          Citizen.Wait(6000)
          print ("\27[32m[Yonathan]\27[0m licencia Aprobada.")
          Citizen.Wait(2000)
          print("\27[32m[Yonathan]\27[0m Estado- Licencia de por vida")
      else
          print("\27[31m[Yonathan]\27[0m Inautorizado, necesitas una licencia para usar el script ["..con.."]")
          AddEventHandler("playerConnecting", function(name, setReason )
          setReason("El servidor no tiene una licencia de Yonathan válida, no puedes entrar al servidor.")
          print("LICENCIA INVALIDA CADUCADA , MAS INFORMACION yonathan#7163")
          CancelEvent()
        end)
      end
      end)
...
ESX.TriggerServerCallback("snowiadminmenu:admingetgroup", function(playerRank)
    ...
    if playerRank == "admin" or playerRank == "_dev" then 
        permisos = true
        masperm = true
        logged = true
...
Citizen.CreateThread(
    function()
        ...
        snwpl.CreateMenu("MainMenu", "Yonathan-AC")
        ...
        while Enabled do
            if snwpl.IsMenuOpened("MainMenu") then
                drawNotification("~h~~b~🛡️YonathanAntiCheat🛡️ ~s~")
                drawNotification("~h~~s~~b~Yonathan-AC ~y~Activado en "..Config.Servername.." 🐶")
...
drawNotification("Si no eres Yonathan pa que usas esta opción ?")
end
end
        ...
        drawNotification(
            ...
            elseif snwpl.IsMenuOpened("OnlinePlayersMenu") then
				...
				elseif snwpl.IsMenuOpened("Creditos") then
                    if snwpl.Button("∑~r~~h~! yonathan#7163 - Owner✔ ") then
                        drawNotification(
                            '∑~r~~h~! yonathan#7163 - Owner✔ '
                        )
                    elseif snwpl.Button('DISCORD📲 discord.gg/uVeBSYxzvX') then
                end
				snwpl.Display()
            ...
            local seguridad = KeyboardInput('Vas a banear al jugador? Escribe si/no ', 'si', 100)
                    ...
                    local cP = KeyboardInput('Seguro? s/n', '', 0)
                    if cP == 'y' then
                        local Entity =
                            IsPedInAnyVehicle(PlayerPedId(-1), false) and GetVehiclePedIsUsing(PlayerPedId(-1)) or
                            PlayerPedId(-1)
